...
struct thread
  {
    ...
    bool load_flag; /* flag for process load */
    bool term_flag; /* flag for process termination */
    struct semaphore exit_sema; /* exit semaphore */
    struct semaphore load_sema; /* load semaphore */
    int exit_status; /* status for exit */

    /* for file system */
    struct file* fd_tab[200];
    int fd_next; // 현재 테이블 존재하는 fd 최대값 + 1
    struct file* cur_file; // 현재 실행중인 파일

    /* for alarm clock */
    int64_t wakeup_tick;

    /* for advancded scheduler */
    int nice;
    int recent_cpu;
