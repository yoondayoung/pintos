...
process_execute (const char *file_name) 
{
  ...
  // extract thread's name
  strtok_r(fn_copy, " ", &save_ptr);
  strlcpy(thr_name, fn_copy, strlen(fn_copy)+1);
  strlcpy(fn_copy, file_name, PGSIZE);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (thr_name, PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  /* if load completed, wake parent process */
...
process_exit (void)
{
  ...
  /* 현재 프로세스에 열린 모든 파일 close */
  if (cur->fd_next > 3) {
      for (int i = 3; i < cur->fd_next; i++) {
...
/* 파일을 프로세스의 fd table에 추가 */
int process_add_fd(struct file* file)
{   
    if (file == NULL) return -1;

    struct thread *thr = thread_current();
    int added_fd = thr->fd_next;
    // 파일 객체-> fd table에 추가
    thr->fd_tab[added_fd] = file;
    // fd table index num 증가
    thr->fd_next++;
    // 추가된 fd 반환
    return added_fd;
}
...
struct file* process_search_file(int fd)
{
    struct thread* thr = thread_current();
    // fd가 유효한지 검사
    if (fd <= 2 || fd >= thr->fd_next) return NULL;
    // 파일 찾아 반환
    struct file* search_file = thr->fd_tab[fd];
    return search_file;
}
...
{   
    struct thread* thr = thread_current();
    // fd가 유효한지 검사
    if (fd <= 2 || fd >= thr->fd_next) return;
    // fd에 해당하는 파일 close
    file_close(thr->fd_tab[fd]);
    // fd tab에 해당 fd 초기화
    thr->fd_tab[fd] = NULL;
}

...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  while (1) {
      argc++;
      if (strtok_r(NULL, " ", &save_ptr) == NULL) break;
  }
